...
public class Localizing implements Behavior
{
	...
	private static int moe = 20;
	private static int heading = 0;				// if starting on corner, then must be 3 (Line 345)  
	...
	public void action() 
	{
		...
		System.out.println("\nP2: " + pose2.getX() + ", " + pose2.getY()); d.msDelay(2000);
		
		//compareDistances(SPLCurrentEdge(pose2.distanceTo(pose1.getLocation())));
		compareDistances(SPLCurrentEdge(Sputnik.opp.getPose().distanceTo(pose1.getLocation()))); 
		localComplete = true;
	...
	private void compareDistances(int edge) 
	{
		...
		//System.out.println("\nPoint: " + Sputnik.opp.getPose().getX() + ", " + Sputnik.opp.getPose().getY());
		System.out.println("\nHead: " + heading);
		d.msDelay(3000);
	}
	...
	private void checkSurrondings(int heading) 
	{
		...
		while (Sputnik.pilot.isMoving()) 
		{
			...
			if (Sputnik.lightSensor.getLightValue() < Sputnik.lightThreshold) 
			{
				...
				{
					localMap.getPosition().getCorridor(temp_heading).setWeight(1);
					//Sputnik.robotMap.getPosition().getCorridor(temp_heading).getNode().getCorridor((temp_heading + 2) % 4).setWeight(1);
				}
			}			
			Thread.yield();
		} 
	...
	{
		//System.out.println("\nIntersect\nC0: " + localMap.getPosition().getCorridor(0).getWeight() + "C1: " + localMap.getPosition().getCorridor(1).getWeight() + "C2: " + localMap.getPosition().getCorridor(2).getWeight() + "C3: " + localMap.getPosition().getCorridor(3).getWeight());
		//d.msDelay(5000);
		
		pose3 = new Pose(Sputnik.opp.getPose().getX(),Sputnik.opp.getPose().getY(),0);
		...
		if(!checkTSection())
			isCorner();
		/*
		if(localMap.getPosition().getCorridor((heading + 1) % 4).getWeight() == 1 && !start)
		{
			distance = pose3.distanceTo(pose1.getLocation());
		...
		else if(localMap.getPosition().getCorridor((heading + 1) % 4).getWeight() == 1)
		{
			Pose pose4 = new Pose(Sputnik.opp.getPose().getX(),Sputnik.opp.getPose().getY(),0);
	...
	private void checkCrossSection() 
	{
		...
		else if(localMap.getPosition().getCorridor(Math.abs((sideCorner + 2) % 4)).getWeight() == 1)
		{
			localComplete = true;
			Sputnik.robotMap.setPosition(new Node(1, 1));
	...
	{
		if(localMap.getPosition().getCorridor(0).getWeight() == 1 && (localMap.getPosition().getCorridor(1).getWeight() == 1 || localMap.getPosition().getCorridor(3).getWeight() == 1))
		{
			corner = true;
		}
		
		else if(localMap.getPosition().getCorridor(2).getWeight() == 1 && (localMap.getPosition().getCorridor(1).getWeight() == 1 || localMap.getPosition().getCorridor(3).getWeight() == 1))
			corner = true;
	}
	...
	private void moveRobot()
	{
		while(corner == false)
		{
			...
			//if(PathSweeping.intersection == true && (Sputnik.lightSensor.getLightValue() > Sputnik.lightThreshold) )
				checkSurrondings(heading);	
			
			PathSweeping.intersection = false;
			if(!corner)Sputnik.pilot.travel(150);
	...
	private void orientToLine()
	{
		...
		{
			// if opposite side line is also weighted as 1, then robot is already on the straight line.
			if(localMap.getPosition().getCorridor(((heading + 2) % 4)).getWeight() != 1)
			{
				Sputnik.pilot.rotate(90);
				heading = (heading + 1) % 4;
			}
	...
	{
		if(localMap.getPosition().getCorridor(1).getWeight() == 1 && localMap.getPosition().getCorridor(0).getWeight() == 1)
			return true;
		
		else if(localMap.getPosition().getCorridor(3).getWeight() == 1 && localMap.getPosition().getCorridor(2).getWeight() == 1)
			return true;
